...
| 5 | Akcni nabidky | projektove | prvni verze hotova | Project agenda pro typy, akční nabídky a odběratele akčních nabídek; typy mají CSS třídu badge `color` stejným principem jako štítky novinek, nabídky mají příznak `is_primary` pro budoucí výběr primárních akcí na hlavní stránce, prohlížeč je primárně nad obrázkovými stranami uloženými v `/media/akce/{id}-{slug}/pages`, nové stránky se nahrávají přes FilePond async/chunk uploader, PDF jsou přesunutá do `/media/akce/{id}-{slug}/` jako download a lokální staging `_files/akce_old` byl po převodu odstraněn. |
...
| 11 | Volani | projektove | prvni verze hotova | Project agenda pro přeúčtování telefonních služeb z původního webu. Nově drží historii: `volani_preuctovani` má unikátní klíč `obdobi + mobil`, zákaznický token `unify`, `volani_souhrn` a `volani_detail` deduplikují importované řádky přes hash a vážou se na stejné `obdobi + mobil`. Administrace umožňuje import původních XML souborů a výpis s odkazy na zákaznický souhrn/detail i e-mailový přehled za více čísel; veřejný e-mailový přehled má výběr období a bez parametru zobrazí poslední dostupné období. |
| 12 | TenisQcup | projektove | prvni verze hotova | Registrace týmů z frontend formuláře, filtrování podle roku a XLSX export vybraných ročníků. |
...
- Verejny web pouziva figmovy model: sedive pozadi dokumentu, uprostred bily page canvas max. `1440px`; horní logo blok a menu jsou full-bleed přes celou šířku viewportu. Jemne oddeleni obsahu se resi bordery na kartach/detailovych blocich, ne samostatnym bilym shellem uvnitr stranky.
- Zalozena hlavni verejna navigace: Letaky, Markety, Velkoobchod, Prodejny, Kariera, O nas dropdown a Kontakty; sekce Nase znacky ma route, ale neni viditelna v hlavnim menu. Rozcestnikove karty pouzivaji ciselne znacky `01`/`02`/`03` ukotvene vlevo nahore; jejich desktopovy rozmer je sjednoceny podle homepage, vyska `104px` a sirka odpovida jedne karte trisloupcoveho rozcestniku.
- Prvni reklamni blok na homepage je plynuly carousel s kartami cca `540x300`, viditelne zhruba 2+ karty; data bere z administrace banneru. Druhy reklamni blok je pevny grid bez carouselu, desktopove pocita s 5 kartami s pomerem blizkym A4 portrait a nizsi vyskou; mobilne se sklada do jednoho sloupce. Pokud pro dany blok neni obsah, blok se nevykresli.
...
- Veřejná stránka `Letáky` odděluje právě platné, nadcházející a uplynulé letáky; horní část má právě platné letáky vlevo a registrační blok odběru vpravo. Načítají se všechny validní a viditelné letáky bez pevného omezení archivu. Přepínání typů je jen u uplynulých letáků, kde je stylované stejně jako štítkové filtry novinek; aktivní filtr zachovává barvu typu a zvýrazní se obrysem. Query parametr `typ` pouze předvybere aktivní typ, ostatní filtry zůstávají viditelné. Karty jsou stránkované bez reloadu stránky a číselné stránkování zobrazuje kompaktní okno max. pěti stránek podobně jako novinky. Breadcrumb používá stejnou ikonu a horní odsazení jako Markety. Karty mají stažení PDF a detail s flip prohlížečem nad obrázkovými stranami z `/media/akce`. Detail letáku otevírá modalový čtecí režim přes téměř celý viewport, viewer využívá maximální viditelnou výšku prohlížeče a podporuje klávesové/boční listování, náhledy stran a lupu se zoomem a posunem; ovládání je plovoucí mimo horní hranu letáku a viewer jde zavřít křížkem nebo klávesou `Esc` přes historii zpět na stránku, odkud byl otevřen, při přímém otevření spadne na přehled letáků. Odběr zapisuje do `rep_akce_users` podle tří hlavních veřejných typů `markety`, `velkoobchod`, `qantoplus` a je chráněný jednotnou frontend captcha ochranou.
...
- Veřejný footer je součástí frontend shellu v `index.php`; obsahuje logo Qanto, skupinovou navigaci `Letáky`, `Pobočky`, `Kariéra`, `Kontakty`, sekci `Ostatní` se statickými texty `/obchodni-podminky`, `/elektronicka-vymena-dat` a `/zakaznicke-karty`, čtyři sociální odkazy řízené přes `stat_vyrazy` (`footer.social.*.url`) a právní odkazy na externí Cookies Správně cookie policy a lokální `/cz/osobni-udaje`. Právní a ostatní statické stránky používají sdílenou šablonu `inc/static_text.php` a obsah z `stat_texty` s kódy `osobni_udaje`, `obchodni_podminky`, `elektronicka_vymena_dat` a `zakaznicke_karty`.
- Frontend nesmi dostavat nove pevne texty natvrdo do PHP sablon ani JS. Pred kazdym novym textem je nutne rozhodnout cil: pevne UI texty patri pojmenovane do `functions/lang.php`, ctou se pres `ui_text()` a administračně se spravují přes DB přepis `ui_texty`; kratke editovatelne texty patri do `stat_vyrazy`, delsi obsahove stranky a clanky do `stat_texty`, texty navazane na konkretni zaznam patri primo do dane datove entity. `functions/lang.php` zustava fallback katalogem pro nasazeni a synchronizaci, administrace `UI texty webu` vklada jen chybejici klice a neprepisuje rucne upravene DB hodnoty. `stat_vyrazy` jsou plain text bez TinyMCE/HTML; HTML obsah patri do `stat_texty`. HTML z TinyMCE se pri ukladani cisti od `<script>` tagu na strane editoru i serveru u sdilenych obsahovych agend. Zmeny `ui_texty`, `stat_vyrazy` a `stat_texty` se v teto fazi zapisuji primo do lokalni DB a do produkce se prenesou z lokalniho stavu; nepridavat pro ne samostatne SQL migrace, pokud nebude receno jinak.
...
| `volani_preuctovani`, `volani_souhrn`, `volani_detail` | `volani_preuctovani`, `volani_souhrn`, `volani_detail` | schema hotove, data/import resit podle XML | Původní tabulky se záměrně jmenují stejně kvůli návaznosti zákaznických odkazů a internímu pojmenování agendy; nová struktura už netruncuje historii. |
...
- Akční nabídky používají vendored JS knihovnu `filepond` v `assets/lib/filepond` (MIT) pro asynchronní a chunk upload obrázkových stran do `/media/akce`.
...
- Po produkčním nasazení ověřit reálné limity webserveru pro dlouhé chunk uploady akčních nabídek a případně doladit velikost chunku.
...
- Akcni nabidky nebudou pokracovat ve stare samostatne `_flip` funkcnosti jako primarnim reseni. Cílová runtime struktura jsou obrázkové stránky v `/media/akce/{id}-{slug}/pages`, které se buď ručně nahrají z administrace, nebo jednorázově převezmou ze stažených `_flip/*/files/mobile`. Výstupní formát a kvalita se řídí systémovými proměnnými `rep_akce_page_output_format` a `rep_akce_page_image_quality`, výchozí je WebP kvalita 82. Pro nové ostré letáky se má z tiskového PDF nahrávat kvalitní zdroj alespoň cca `1720 x 2400 px`; malé stránky kolem `860 x 1200 px` budou na Retina displejích a při lupě měkčí. Pro historii se celý `_flip` balík nestahuje; downloader bere jen `config.js` a velké obrázky `files/mobile/*.jpg`. PDF je pouze download v `/media/akce/{id}-{slug}/`; po převodu byl staging `_files/akce_old` odstraněn.
- Pro listovací prohlížeč akčních nabídek se používá vendored knihovna `page-flip` v `assets/lib/page-flip` (MIT). Viewer v administraci pracuje nad obrázkovými stránkami: nejdříve `rep_akce_strany`, potom nouzově stažené `_flip/files/mobile/*.jpg`, pokud ještě nebyl spuštěn převod do `/media/akce`. Nové nabídky se nahrávají jako sada obrázků stran přes vendored FilePond uploader (MIT), který posílá soubory samostatně a podporuje chunky; PDF je doplňkový download.
