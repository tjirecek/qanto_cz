 ...
 */
function rep_ples_export_columns(): array
{
    return [
        'id' => 'ID',
        'rok' => 'Rok',
...
function rep_ples_export_filename(array $years, bool $none, int $valid): string
{
    $suffix = $none ? 'zadne-roky' : ($years === [] ? 'vsechny-roky' : 'roky-' . implode('-', $years));
    $validSuffix = $valid === 1 ? 'validni' : 'nevalidni';
    return 'ples-registrace-' . $validSuffix . '-' . $suffix . '-' . date('Ymd-His') . '.xlsx';
}
...
try {
    ...
    $years = rep_ples_parse_years($_GET['years'] ?? []);
    $rows = $showNoYears ? [] : rep_ples_rows($pdo, $years, $valid);
    $columns = rep_ples_export_columns();
    ...
    header('Content-Disposition: attachment; filename="' . rep_ples_export_filename($years, $showNoYears, $valid) . '"');
