...
$notice = '';
$yearRows = [];
$selectedYears = rep_tenis_qcup_parse_years($_GET['years'] ?? []);
...
$rows = [];
$csrfToken = (string)admin_session_get('rep_tenis_qcup_csrf_token', '');
if ($csrfToken === '') {
    $csrfToken = bin2hex(random_bytes(16));
    admin_session_set('rep_tenis_qcup_csrf_token', $csrfToken);
}
...
try {
    ...
    if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
        ...
        if ($action === 'invalidate' || $action === 'validate') {
            ...
            rep_tenis_qcup_set_valid($pdo, (int)($_POST['id'] ?? 0), $newValid);
            $notice = $newValid === 1 ? 'Registrace byla obnovena.' : 'Registrace byla znevalidněna.';
            $selectedYears = rep_tenis_qcup_parse_years($_POST['years'] ?? []);
    ...
    $rows = $showNoYears ? [] : rep_tenis_qcup_rows($pdo, $selectedYears, $valid);
} catch (Throwable $e) {
    $error = $e->getMessage();
}

$selectedYearMap = array_fill_keys($selectedYears, true);
$exportUrl = rep_tenis_qcup_export_url($selectedYears, $showNoYears, $valid);
$validToggleParams = [
    'section' => '02',
    'page' => '07',
    'sec_page' => '01',
...
$validCount = ($pdo instanceof PDO) ? rep_tenis_qcup_count($pdo, 1) : 0;
$invalidCount = ($pdo instanceof PDO) ? rep_tenis_qcup_count($pdo, 0) : 0;
?>

<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <div>
        <h1 class="h3 mb-1 text-gray-800">TenisQcup</h1>
        <div class="text-muted small">Project agenda qanto.cz: registrace týmů na tenisový turnaj z frontend formuláře.</div>
    </div>

    <div class="d-flex flex-wrap gap-2 mt-3 mt-sm-0">
        <?php if ($valid === 1): ?>
            <a href="<?= rep_tenis_qcup_e($validToggleUrl) ?>" class="btn btn-sm btn-danger shadow-sm">zobrazit nevalidní <i class="bi bi-slash-circle ms-1"></i></a>
        <?php else: ?>
            <a href="<?= rep_tenis_qcup_e($validToggleUrl) ?>" class="btn btn-sm btn-outline-primary shadow-sm">zobrazit validní <i class="bi bi-arrow-repeat ms-1"></i></a>
        <?php endif; ?>
...
<?php if ($error !== ''): ?>
    <div class="alert alert-danger">
        <?= rep_tenis_qcup_e($error) ?>
    </div>
<?php else: ?>
    <?php if ($notice !== ''): ?>
        <div class="alert alert-success d-flex align-items-center" role="alert">
            <i class="bi bi-check-circle me-2"></i>
            <div><?= rep_tenis_qcup_e($notice) ?></div>
        </div>
    <?php endif; ?>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex flex-wrap align-items-start justify-content-between gap-2">
            <div class="flex-grow-1">
                ...
                <form method="get" class="d-flex flex-wrap gap-2 align-items-center mt-2" data-rep-tenis-qcup-filter>
                    ...
                    <?php foreach ($yearRows as $yearRow): ?>
                        ...
                        ?>
                        <input type="checkbox" class="btn-check" name="years[]" value="<?= $year ?>" id="tenisYear<?= $year ?>" <?= $checked ? 'checked' : '' ?>>
                        <label class="btn btn-outline-primary btn-sm" for="tenisYear<?= $year ?>"><?= $year ?><span class="badge text-bg-light ms-1"><?= (int)$yearRow['total'] ?></span></label>
                    <?php endforeach; ?>
                    <button type="submit" class="btn btn-sm btn-primary"><i class="bi bi-funnel me-1"></i> Filtrovat</button>
                    <a class="btn btn-sm btn-outline-secondary" href="index.php?section=02&amp;page=07&amp;sec_page=01&amp;all=1&amp;valid=<?= (int)$valid ?>">Všechny</a>
                    <a class="btn btn-sm btn-outline-secondary" href="index.php?section=02&amp;page=07&amp;sec_page=01&amp;none=1&amp;valid=<?= (int)$valid ?>">Žádné</a>
                </form>
            </div>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-striped table-hover table-bordered table-sm js-datatable align-middle w-100" data-order='[[ 1, "desc" ], [ 2, "desc" ]]' data-page-length="500">
                    ...
                    <tr>
                        ...
                        <th class="text-filter dt-autocomplete">Tým</th>
                        <th class="text-filter dt-autocomplete">Hráč 1</th>
                        <th class="text-filter">E-mail 1</th>
                        <th class="text-filter">Mobil 1</th>
                        <th class="text-filter dt-autocomplete">Hráč 2</th>
                        <th class="text-filter">E-mail 2</th>
                        <th class="text-filter">Mobil 2</th>
                    ...
                    <?php foreach ($rows as $row): ?>
                        <?php
                        $player1 = trim((string)($row['name1'] ?? '') . ' ' . (string)($row['surname1'] ?? ''));
                        $player2 = trim((string)($row['name2'] ?? '') . ' ' . (string)($row['surname2'] ?? ''));
                        ?>
                        <tr>
                            <td><?= (int)$row['id'] ?></td>
                            <td><?= (int)$row['rok'] ?></td>
                            <td class="text-nowrap" data-order="<?= rep_tenis_qcup_e($row['datum'] ?? '') ?>"><?= rep_tenis_qcup_e($row['datum'] ?? '') ?></td>
                            <td class="fw-semibold"><?= rep_tenis_qcup_e($row['team_name'] ?? '') ?></td>
                            <td><?= rep_tenis_qcup_e($player1) ?></td>
                            <td><?= rep_tenis_qcup_e($row['email1'] ?? '') ?></td>
                            <td><?= rep_tenis_qcup_e($row['mobil1'] ?? '') ?></td>
                            <td><?= rep_tenis_qcup_e($player2) ?></td>
                            <td><?= rep_tenis_qcup_e($row['email2'] ?? '') ?></td>
                            <td><?= rep_tenis_qcup_e($row['mobil2'] ?? '') ?></td>
                            <td><?= rep_tenis_qcup_e($row['pozval'] ?? '') ?></td>
                            <td class="text-break"><?= nl2br(rep_tenis_qcup_e($row['poznamka'] ?? '')) ?></td>
                            <td data-order="<?= rep_tenis_qcup_e($row['ts_u'] ?? '') ?>"><?= rep_tenis_qcup_updated_cell($row) ?></td>
                            ...
                            <td class="text-center">
                                <?php if ($valid === 1): ?>
                                    <form method="post" class="d-inline" data-confirm="Opravdu znevalidnit tuto registraci?">
                                        <input type="hidden" name="csrf_token" value="<?= rep_tenis_qcup_e($csrfToken) ?>">
